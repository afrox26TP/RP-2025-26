	...
	for prov_id in province_db.keys():
		...
		if owner == "SEA" or type == "sea":
			...
			if mod == "political":
				...
				var max_pop_hranice = 3000000.0 
				var sytost = clamp(pop / max_pop_hranice, 0.0, 1.0)
				barva = Color(sytost, 0.2, 0.2, 1.0)
				
			elif mod == "gdp":
				var gdp = float(prov_data.get("gdp", 0.0))
				var max_gdp_hranice = 500.0
				var sytost = clamp(gdp / max_gdp_hranice, 0.0, 1.0)
